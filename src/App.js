...
function App() {
	...
	useEffect(() => {
		...
		if (token) {
			setIsLoggedIn(true);
			axios
				.post(`${process.env.REACT_APP_SERVER}/users/getuser`, { jwt: token })
			...
			{loading ? (
				...
				<div className="App">
					...
					<Routes>
						...
						<Route
							path="/profile"
							element={<Profile isLoggedIn={isLoggedIn} userInfo={userInfo} />}
						/>
						...
						<Route
							path="/accomodation"
							element={<Accomodation isLoggedIn={isLoggedIn} userInfo={userInfo} />}
						/>
						<Route path="/campus-ambassador" element={<BrandAmbassador />} />
						<Route path="/how-to-register" element={<HowToRegister />} />
						<Route
							path="/day-selection"
							element={<DaySelection day={day} setDay={setDay} />}
						/>
					</Routes>
				</div>
			)}
